...
@[instance_reducible]
noncomputable def finiteAlgebraFormationsCompleteLattice {S : Type u} [Finite S]
    (Sig : Signature S) :
  ...
  (carrierOrderIso (algebraFinite Sig) (finiteAlgebraFormations Sig)
    (finiteAlgebraFormations_isAlgebraicClosureSystem Sig)).symm.toGaloisInsertion.liftCompleteLattice

/-- `B-C011` (`FormAlgFAlgLat`): `Form_Alg_f(Σ)`, ordered by inclusion, is an
algebraic lattice. -/
theorem finiteAlgebraFormations_isAlgebraicLattice {S : Type u} [Finite S]
    (Sig : Signature S) :
  ...
  have h := isAlgebraicLattice_of_orderIso
    (carrierOrderIso (algebraFinite Sig) (finiteAlgebraFormations Sig)
      (finiteAlgebraFormations_isAlgebraicClosureSystem Sig)).symm
...
/-! ### `B-X001`: cyclic and periodic algebras. -/

/-- `B-X001`: `X` is a *cyclic subalgebra* of `A` when it is generated by a
single element: `Sg_A(δ^{t,a}) = X` for some sort `t` and `a : A_t`. -/
def IsCyclicSub {S : Type u} (Sig : Signature S) (A : Alg Sig) (X : Sub A.1) : Prop :=
  ∃ (t : S) (a : A.1 t), Sg Sig A.2 (deltaSub t ({a} : Set (A.1 t))) = X

/-- `B-X001`: a `Σ`-algebra is *periodic* when every cyclic subalgebra of it is
finite. -/
def IsPeriodicAlg {S : Type u} (Sig : Signature S) (A : Alg Sig) : Prop :=
  ∀ (X : Sub A.1) (hX : IsSubalgebra Sig A.2 X),
    IsCyclicSub Sig A X → FiniteAlg (subAlg Sig A.2 X hX)

/-- `B-X001`: `F_p`, the set of periodic `Σ`-algebras. -/
def periodicAlgebras {S : Type u} (Sig : Signature S) : Set (Alg Sig) :=
  {A | IsPeriodicAlg Sig A}

/-- The direct image of the concentrated subset `δ^{t,a}` is `δ^{t,f a}`. -/
theorem directImage_deltaSub {S : Type u} {A B : SSet S} (f : SortedMap A B)
    (t : S) (a : A t) :
    directImage f (deltaSub t ({a} : Set (A t))) =
      deltaSub t ({f t a} : Set (B t)) := by
  funext s
  by_cases h : s = t
  · subst h
    simp only [directImage, deltaSub_self, Set.image_singleton]
  · simp only [directImage, deltaSub_of_ne h, Set.image_empty]

/-- For a homomorphism `f`, generation commutes with direct image:
`f[Sg_A(X)] = Sg_B(f[X])`. -/
theorem directImage_Sg {S : Type u} (Sig : Signature S) {A B : SSet S}
    (FA : AlgStruct Sig A) (FB : AlgStruct Sig B) (f : SortedMap A B)
    (hf : IsAlgHom Sig FA FB f) (X : Sub A) :
    directImage f (Sg Sig FA X) = Sg Sig FB (directImage f X) := by
  funext s
  ext b
  constructor
  · rintro ⟨a, ha, rfl⟩
    induction ha with
    | hyp s a hax => exact subset_Sg Sig FB (directImage f X) s ⟨a, hax, rfl⟩
    | op p σ a _ ih =>
        rw [hf p σ a]
        exact MemSg.op p σ (fun i => f (p.1.get i) (a i)) (fun i => ih i)
  · intro hb
    induction hb with
    | hyp s b hb =>
        obtain ⟨a, ha, rfl⟩ := hb
        exact ⟨a, subset_Sg Sig FA X s ha, rfl⟩
    | op p σ b _ ih =>
        choose a ha hfa using ih
        refine ⟨FA p σ a, MemSg.op p σ a (fun i => ha i), ?_⟩
        rw [hf p σ a]
        congr 1
        funext i
        exact hfa i

/-- A finite product of finite `Σ`-algebras is finite. The `Finite S` hypothesis
covers the empty product, the final algebra `1` (which is finite exactly when `S`
is). -/
theorem finiteSSet_iAlg {S : Type u} (Sig : Signature S) [Finite S] {ι : Type u}
    [Fintype ι] (C : ι → Alg Sig) (hC : ∀ i, FiniteAlg (C i)) :
    FiniteAlg (iAlg Sig C) := by
  unfold FiniteAlg FiniteSSet at *
  haveI : Finite (Sigma (fun _ : S => PUnit.{u+1})) := by
    refine Finite.of_surjective
      (fun s : S => (⟨s, PUnit.unit⟩ : Sigma (fun _ : S => PUnit.{u+1}))) ?_
    intro x
    obtain ⟨s, u⟩ := x
    exact ⟨s, congrArg (fun u => (⟨s, u⟩ : Sigma (fun _ : S => PUnit.{u+1})))
      (Subsingleton.elim u PUnit.unit)⟩
  haveI : ∀ i, Fintype (Sigma (C i).1) := fun i => by
    haveI : Finite (Sigma (C i).1) := hC i
    exact Fintype.ofFinite (Sigma (C i).1)
  haveI : Finite (∀ i, Sigma (C i).1) := inferInstance
  refine Finite.of_injective
    (fun x : Sigma (iAlg Sig C).1 =>
      ((⟨x.1, PUnit.unit⟩ : Sigma (fun _ : S => PUnit.{u+1})),
       fun i => (⟨x.1, x.2 i⟩ : Sigma (C i).1))) ?_
  rintro ⟨s, g⟩ ⟨s', g'⟩ h
  have hs : s = s' := congrArg (fun z => z.1.1) h
  subst hs
  have hg : g = g' := by
    funext i
    have h2 : (⟨s, g i⟩ : Sigma (C i).1) = ⟨s, g' i⟩ :=
      congrFun (congrArg Prod.snd h) i
    exact eq_of_heq ((Sigma.mk.injEq _ _ _ _).mp h2).2
  subst hg
  rfl

/-- Finiteness transfers along a componentwise-injective sorted map. -/
theorem finiteSSet_of_injective {S : Type u} {A B : SSet S} (f : SortedMap A B)
    (hf : ∀ s, Function.Injective (f s)) (hB : FiniteSSet B) : FiniteSSet A := by
  unfold FiniteSSet at *
  haveI : Finite (Sigma B) := hB
  refine Finite.of_injective
    (fun x : Sigma A => (⟨x.1, f x.1 x.2⟩ : Sigma B)) ?_
  rintro ⟨s, a⟩ ⟨s', a'⟩ h
  have hs : s = s' := congrArg Sigma.fst h
  subst hs
  have hfa : f s a = f s a' :=
    eq_of_heq ((Sigma.mk.injEq _ _ _ _).mp h).2
  exact congrArg (fun a => (⟨s, a⟩ : Sigma A)) (hf s hfa)

/-- `B-X001`: `F_p` is closed under homomorphic images. An epimorphic image of a
periodic algebra is periodic: a cyclic subalgebra `Sg_B(δ^{t,c})` of `B` is the
direct image of the cyclic subalgebra `Sg_A(δ^{t,a})` of `A` (where `f a = c`),
which is finite. -/
theorem periodicAlgebras_closed_HOperator {S : Type u} (Sig : Signature S) :
    HOperator Sig (periodicAlgebras Sig) ⊆ periodicAlgebras Sig := by
  rintro B ⟨A, hA, f, hf⟩
  simp only [periodicAlgebras, Set.mem_ofPred_eq] at hA ⊢
  intro Y hY hcyc
  obtain ⟨t, c, hYc⟩ := hcyc
  obtain ⟨a, ha⟩ := hf.2 t c
  let X : Sub A.1 := Sg Sig A.2 (deltaSub t ({a} : Set (A.1 t)))
  have hX : IsSubalgebra Sig A.2 X := Sg_isSubalgebra Sig A.2 _
  have hcycA : IsCyclicSub Sig A X := ⟨t, a, rfl⟩
  have hfinA : FiniteAlg (subAlg Sig A.2 X hX) := hA X hX hcycA
  have hYeq : Y = directImage f X := by
    rw [← hYc, ← ha, ← directImage_deltaSub f t a,
      directImage_Sg Sig A.2 B.2 f hf.1]
  let g : SortedMap (subAlg Sig A.2 X hX).1 (subAlg Sig B.2 Y hY).1 :=
    fun s x => ⟨f s x.1, by rw [hYeq]; exact ⟨x.1, x.2, rfl⟩⟩
  have hg : ∀ s, Function.Surjective (g s) := by
    intro s y
    have hy : y.1 ∈ directImage f X s := by rw [← hYeq]; exact y.2
    obtain ⟨a', haX, haf⟩ := hy
    exact ⟨⟨a', haX⟩, Subtype.ext haf⟩
  unfold FiniteAlg FiniteSSet at hfinA ⊢
  refine Finite.of_surjective
    (fun x : Sigma (subAlg Sig A.2 X hX).1 =>
      (⟨x.1, g x.1 x.2⟩ : Sigma (subAlg Sig B.2 Y hY).1)) ?_
  rintro ⟨s, y⟩
  obtain ⟨x, hx⟩ := hg s y
  exact ⟨⟨s, x⟩,
    congrArg (fun z => (⟨s, z⟩ : Sigma (subAlg Sig B.2 Y hY).1)) hx⟩

/-- `B-X001`: `F_p` is closed under finite subdirect products (under `Finite S`,
which the empty product `1` requires). If `f : A → ∏ C^α` is subdirect and each
`C^α` is periodic, a cyclic subalgebra `X = Sg_A(δ^{t,d})` maps onto
`Sg_{∏C}(δ^{t,fd}) ⊆ ∏ Sg_{C^α}(δ^{t,(fd)^α})`, a subobject of a finite product
of finite algebras, so `X` is finite. -/
theorem periodicAlgebras_closed_PFsdOperator {S : Type u} (Sig : Signature S)
    [Finite S] : PFsdOperator Sig (periodicAlgebras Sig) ⊆ periodicAlgebras Sig := by
  rintro A ⟨ι, hι, C, hC, f, hf⟩
  haveI : Fintype ι := hι
  simp only [periodicAlgebras, Set.mem_ofPred_eq] at hC ⊢
  intro X hX hcyc
  obtain ⟨t, d, hXc⟩ := hcyc
  let Di : ι → Alg Sig := fun i => subAlg Sig (C i).2
    (Sg Sig (C i).2 (deltaSub t ({(f t d) i} : Set ((C i).1 t))))
    (Sg_isSubalgebra Sig (C i).2 _)
  have hDi : ∀ i, FiniteAlg (Di i) := fun i =>
    hC i _ (Sg_isSubalgebra Sig (C i).2 _) ⟨t, (f t d) i, rfl⟩
  have hprod : FiniteAlg (iAlg Sig Di) := finiteSSet_iAlg Sig Di hDi
  have hXeq : directImage f X =
      Sg Sig (iAlg Sig C).2 (deltaSub t ({f t d} : Set ((iAlg Sig C).1 t))) := by
    rw [← hXc, directImage_Sg Sig A.2 (iAlg Sig C).2 f hf.1.1,
      directImage_deltaSub f t d]
  have hsub : ∀ s (y : (iAlg Sig C).1 s), y ∈ directImage f X s →
      ∀ i, y i ∈ Sg Sig (C i).2 (deltaSub t ({(f t d) i} : Set ((C i).1 t))) s := by
    intro s y hy
    rw [hXeq] at hy
    induction hy with
    | hyp s y hy =>
        intro i
        have hgen : y i ∈ deltaSub t ({(f t d) i} : Set ((C i).1 t)) s := by
          by_cases hst : s = t
          · subst hst
            simp only [deltaSub_self, Set.mem_singleton_iff] at hy ⊢
            rw [hy]
          · rw [deltaSub_of_ne hst] at hy
            exact absurd hy (Set.notMem_empty y)
        exact subset_Sg Sig (C i).2 (deltaSub t ({(f t d) i})) s hgen
    | op p σ b _ ih =>
        intro i
        exact MemSg.op p σ (fun j => b j i) (fun j => ih j i)
  have hfX : FiniteSSet (fun s => {y : (iAlg Sig C).1 s // y ∈ directImage f X s}) := by
    unfold FiniteSSet at hprod ⊢
    haveI : Finite (Sigma (fun _ : S => PUnit.{u+1})) := by
      refine Finite.of_surjective
        (fun s : S => (⟨s, PUnit.unit⟩ : Sigma (fun _ : S => PUnit.{u+1}))) ?_
      intro x
      ...
      exact ⟨s, congrArg (fun u => (⟨s, u⟩ : Sigma (fun _ : S => PUnit.{u+1})))
        (Subsingleton.elim u PUnit.unit)⟩
    haveI : ∀ i, Fintype (Sigma (Di i).1) := fun i => by
      haveI : Finite (Sigma (Di i).1) := hDi i
      exact Fintype.ofFinite (Sigma (Di i).1)
    haveI : Finite (∀ i, Sigma (Di i).1) := inferInstance
    refine Finite.of_injective
      (fun x : Sigma (fun s => {y : (iAlg Sig C).1 s // y ∈ directImage f X s}) =>
        ((⟨x.1, PUnit.unit⟩ : Sigma (fun _ : S => PUnit.{u+1})),
         fun i => (⟨x.1, (⟨x.2.1 i, hsub x.1 x.2.1 x.2.2 i⟩ : (Di i).1 x.1)⟩ :
           Sigma (Di i).1))) ?_
    ...
    have hyy : y = y' := by
      funext i
      have h2 : (⟨s, (⟨y i, hsub s y hy i⟩ : (Di i).1 s)⟩ : Sigma (Di i).1) =
                ⟨s, (⟨y' i, hsub s y' hy' i⟩ : (Di i).1 s)⟩ :=
        congrFun (congrArg Prod.snd h) i
      have h3 : (⟨y i, hsub s y hy i⟩ : (Di i).1 s) = ⟨y' i, hsub s y' hy' i⟩ :=
        eq_of_heq ((Sigma.mk.injEq _ _ _ _).mp h2).2
      exact congrArg Subtype.val h3
    subst hyy
    exact congrArg
      (fun z => (⟨s, z⟩ : Sigma (fun s => {y : (iAlg Sig C).1 s // y ∈ directImage f X s})))
      (Subtype.ext rfl)
  exact finiteSSet_of_injective
    (fun s (a : {a : A.1 s // a ∈ X s}) =>
      (⟨f s a.1, ⟨a.1, a.2, rfl⟩⟩ : {y : (iAlg Sig C).1 s // y ∈ directImage f X s}))
    (fun s a a' h => Subtype.ext (hf.1.2 s (congrArg Subtype.val h)))
    hfX

/-- `B-X001`: `F_p`, the periodic `Σ`-algebras, is a formation of `Σ`-algebras.
The `Finite S` hypothesis is needed for the empty subdirect product: the final
algebra `1` is finite exactly when `S` is, and `1 ∈ P_fsd(F_p)` for any `F_p`, so
without it `F_p` is not a formation. -/
theorem periodicAlgebras_isAlgebraFormation {S : Type u} (Sig : Signature S) [Finite S] :
    IsAlgebraFormation Sig (periodicAlgebras Sig) :=
  ⟨periodicAlgebras_closed_HOperator Sig, periodicAlgebras_closed_PFsdOperator Sig⟩
