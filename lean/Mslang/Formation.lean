...
/-! ### `B-C005`: `Form_Alg(Σ)` is an algebraic lattice. -/

/-- `B-C005`: the closure operator on `Sub(Alg(Σ))` whose closed sets are the
formations of `Σ`-algebras (`B-P018`). -/
noncomputable def algebraFormationsClosureOperator {S : Type u} (Sig : Signature S) :
    ClosureOperator (Set (Alg Sig)) :=
  ClosureOperator.ofCompletePred (algebraFormations Sig) (by
    intro s hs
    by_cases hne : s.Nonempty
    · change (⋂₀ s) ∈ algebraFormations Sig
      exact (algebraFormations_isAlgebraicClosureSystem Sig).2.1 s hs hne
    · have he : s = ∅ := Set.not_nonempty_iff_eq_empty.mp hne
      subst he
      change (⋂₀ (∅ : Set (Set (Alg Sig)))) ∈ algebraFormations Sig
      rw [Set.sInter_empty]
      exact (algebraFormations_isAlgebraicClosureSystem Sig).1)

/-- `B-C005`: `Form_Alg(Σ)` is a complete lattice. -/
@[instance_reducible]
noncomputable def algebraFormationsCompleteLattice {S : Type u} (Sig : Signature S) :
    CompleteLattice (algebraFormations Sig) :=
  (algebraFormationsClosureOperator Sig).gi.liftCompleteLattice

/-- `B-C005` (`FormAlgAlgLat`): `Form_Alg(Σ)`, ordered by inclusion, is an
algebraic lattice: the formations generated by finite sets of algebras
(`Fmg_Σ(M)`, `M` finite) are exactly the compact elements, and every formation is
the join of the compact elements below it. -/
theorem algebraFormations_isAlgebraicLattice {S : Type u} (Sig : Signature S) :
    @IsAlgebraicLattice (algebraFormations Sig) (algebraFormationsCompleteLattice Sig) := by
  have h := isAlgebraicLattice_of_isAlgebraicClosureOperator
    (algebraFormationsClosureOperator Sig)
    (fun s hs hne hdir =>
      (algebraFormations_isAlgebraicClosureSystem Sig).2.2 s hs hne hdir)
  change @IsAlgebraicLattice (algebraFormationsClosureOperator Sig).Closeds
    ((algebraFormationsClosureOperator Sig).gi.liftCompleteLattice)
  exact h

/-- `B-C005` (compact characterization): a formation is compact if and only if it
is the formation generated by a finite set of `Σ`-algebras, i.e. the closure
`Fmg_Σ(M)` (here `(algebraFormationsClosureOperator Sig).toCloseds M`) of a
finite `M`. -/
theorem algebraFormations_isCompact_iff {S : Type u} (Sig : Signature S)
    (F : algebraFormations Sig) :
    @IsCompact (algebraFormations Sig) (algebraFormationsCompleteLattice Sig) F ↔
      ∃ M : Set (Alg Sig), M.Finite ∧
        F = (algebraFormationsClosureOperator Sig).toCloseds M := by
  have h := isCompact_iff_exists_finite_closure (algebraFormationsClosureOperator Sig)
    (fun s hs hne hdir =>
      (algebraFormations_isAlgebraicClosureSystem Sig).2.2 s hs hne hdir)
  have hF := h F
  change @IsCompact (algebraFormationsClosureOperator Sig).Closeds
      ((algebraFormationsClosureOperator Sig).gi.liftCompleteLattice) F ↔
    ∃ M : Set (Alg Sig), M.Finite ∧
