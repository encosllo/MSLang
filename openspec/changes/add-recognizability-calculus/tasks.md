...
- [ ] 2.4 Prove the `T`-indexed characterization (`Ω^A([L, ∅^{S−T}])` of finite index); verify the `S−T` extension is built with `Mslang`'s delta/empty-family machinery. *Deferred: needs the `A↾_T` restriction encoding (design D2).*
...
- [ ] 3.2 Prove Boolean closure of `Rec(A)`/`Rec_s(A)` (`∅`/`A_s`, union, intersection, relative complement); verify each clause is axiom-clean. *Partial: union/intersection/complement done (`recognizable_union`/`_inter`/`_compl`); the `∅`/`A_s` bounds are deferred — the existential definition is vacuous without a finite-index congruence, so the bound likely needs a finiteness hypothesis (flagged, not guessed).*
