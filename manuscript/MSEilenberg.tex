...
\providecommand{\lean}[1]{}       % blueprint: Lean declaration name
...
\blockid{B-X001}
\begin{examples}
Let $\Sigma$ be an $S$-sorted signature and $\mathbf{C}$ a $\Sigma$-algebra. We call $\mathbf{C}$ \emph{cyclic} if there exists a $t\in S$ and a $c\in C_{t}$ such that $\mathrm{Sg}_{\mathbf{C}}(\delta^{t,c}) = C$. On the other hand, we call a $\Sigma$-algebra $\mathbf{A}$ \emph{periodic} if all its cyclic subalgebras are finite. We denote by $\mathcal{F}_{p}$ the set of all periodic $\Sigma$-algebras. The set $\mathcal{F}_{p}$ is a formation of $\Sigma$-algebras. Let us check that $\mathrm{H}(\mathcal{F}_{p})\subseteq \mathcal{F}_{p}$ and $\mathrm{P}_{\mathrm{fsd}}(\mathcal{F}_{p})\subseteq \mathcal{F}_{p}$.
